...
5. `USER/Rep/system/system_port.h`
...
如果检测到未知模式，`system_port.c` 会记录 warning，并把模式重置回 `eSYSTEM_INIT_MODE`。
...
console 接线在 `system_port.c` 的 `initializeConsole()` 中完成，当前顺序如下：
...
### 6.2 `system_port.h` 的职责

`system_port.h` 当前承载的是 system 任务参数和任务入口声明，属于应用编排配置层。
...
- `system_port.c` 是启动编排层，不要把具体驱动实现写进这里。
...
2. 阅读 `USER/Rep/system/system_port.h`，确认当前任务参数和周期。
3. 阅读 `USER/Rep/system/system_port.c`，确认任务创建路径和模式切换流程。
...
`system` 是当前工程的系统编排层：`system.c` 管模式，`system_port.c` 管任务和启动接线，`system_debug.c` 管 system 调试命令。后续扩展时保持这三层边界稳定，AI 才能持续按当前架构工作。